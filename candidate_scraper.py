...
class CandidateTable:
    ...
    #TODO: add function to find desired table
    #TODO: use (url,state) as input.
    ...
    def __init__(self,url,state,overwrite=False):
        ...
        self.get_election_and_file_name()
        ...
        if self.get_table():
            self.get_party_index()
            self.get_district_rows()
    ...
    def get_election_and_file_name(self):
        ...
        self.election = ' '.join(election)
        self.file_name = 'candidates_{}.json'.format('_'.join(election))
...
class AllCandidatesToCSV:
    ...
    def output_to_csv(self):
        with open(self.output_file_prefix+'.csv','w') as file:
            fieldnames = ['name','score','keywords','election','district','primary_date','incumbent','ballotpedia','websites']
...
if __name__=='__main__':
    StateLegSearcher(overwrite=True)